...
protocol RootPresentable: class {
    var listener: RootPresentableListener? { get set }
    func insertColorSet(at indexPath: IndexPath)
}

class RootInteractor: RootPresentableListener {
    
    let presenter: RootPresentable
    
    private let colorManager: ColorManaging
    
    private var colorSetList: [ColorSet] = []
    
    init(presenter: RootPresentable, colorManager: ColorManaging) {
